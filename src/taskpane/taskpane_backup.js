...
export async function run() {
  ...
  try {
    await PowerPoint.run(async (context) => {
      ...
      // ② 正規表現を「全文マッチ」にしておく（必要があれば後で絞り込み）
      const re = /.*/;
      ...
      for (let slideIndex = 0; slideIndex < slides.items.length; slideIndex++) {
        ...
        for (let shapeIndex = 0; shapeIndex < shapes.items.length; shapeIndex++) {
          ...
          if (!matchedLine) {
            logToScreen(`      ● マッチ行なしのためスキップ`);
            continue;
          }
          logToScreen(`      ▶ 検出テキスト: "${matchedLine}"`);

          // ⑤ Jina（DeepSearch）API を呼び出す
          const jinaResult = await callJinaFactCheck(matchedLine);
          logToScreen(`      ▶ Jina API レスポンス: ${JSON.stringify(jinaResult)}`);
          
          // エラーが発生した場合も結果を表示
          if (!jinaResult || !jinaResult.hit) {
            logToScreen(`      ● Jina でヒットしなかったためエラー表示`);
            displayFactCheckError(slideIndex + 1, shapeIndex + 1, matchedLine, jinaResult?.error);
            continue;
          }
          
          // UI に結果を表示
          displayFactCheckResult(slideIndex + 1, shapeIndex + 1, matchedLine, jinaResult);

          // ⑥ verdict に “false” を含むかどうかで文字色を決定
          //     true/false 以外に「曖昧」な日本語解説が返ってくる場合は都度判定ロジックを追加してください
          const result = jinaResult.result;
          let fontColor;
...
////////////////////////////////////////////////////////////////////////////////
// displayFactCheckResult(): ファクトチェック結果をUIに表示する関数
function displayFactCheckResult(slideNum, shapeNum, claim, result) {
  const resultsContainer = document.getElementById("resultsContainer");
